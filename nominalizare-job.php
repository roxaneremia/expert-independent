...
session_start();
  ...
  if(isset($_POST['nominalizeaza'])) {
    $concurs =  new Concurs();
    $concurs->nominalizareConcurs($_POST['optradio'],$_GET['id_job']);
    //echo '<pre>'; print_r($_POST); echo '</pre>'; die();
  }
  else
  {
    ?>
      <!--<script>alert("Nu ai putut sa nominalizezi");</script>-->
    <?php
  }
...
<div class="container" style="margin-top: 3%">
  ...
  <div class="jumbotron">
    ...
    while($row = mysqli_fetch_assoc($result)) 
      ...
      <div class="radio">
        <label><input type="radio" name="optradio" value="<?php echo $row['id_membru']; ?>"><?php echo $row['clientp']." ".$row['clientn']; ?></label>
      </div>
     <?php } ?>

    <input class="large-btn" value="nominalizeaza" type="submit" name="nominalizeaza"/>
  </form>
